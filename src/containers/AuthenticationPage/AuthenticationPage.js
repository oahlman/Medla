...
export class AuthenticationPageComponent extends Component {
  ...
  render() {
    ...
    const handleSubmitLogin = values => {
      ...
      const emailParams = { email: email, ...rest };
      const phoneParams = { email: `phone+${email}@medla.app`, ...rest };
    ...
    const authenticationForms = (
      <div className={css.content}>
        ...
        {isLogin ? (
          <LoginForm className={css.loginForm} onSubmit={handleSubmitLogin} inProgress={authInProgress} />
        ) : (
          <SignupForm
    ...
    const phoneNumber = currentUserLoaded ? <span className={css.email}>{user.attributes.email.replace(/phone\+|@medla.app/g, "")}</span> : email;
      ...
      <div className={css.content}>
        <NamedLink className={css.verifyClose} name="LandingPage">
          <span className={css.closeText}>
            <FormattedMessage id="AuthenticationPage.verifyPhoneClose" />
          </span>
          <IconClose rootClassName={css.closeIcon} />
        </NamedLink>
        <IconEmailSent className={css.modalIcon} />
        <h1 className={css.modalTitle}>
          <FormattedMessage id="AuthenticationPage.verifyPhoneTitle" values={{ name }} />
        </h1>
        <p className={css.modalMessage}>
          <FormattedMessage id="AuthenticationPage.verifyPhoneText" values={{ phoneNumber }} />
        </p>
        {resendErrorMessage}
        ...
        <div className={css.bottomWrapper}>
          <p className={css.modalHelperText}>
            {sendVerificationEmailInProgress ? (
              ...
              <FormattedMessage id="AuthenticationPage.resendTextMessage" values={{ resendEmailLink }} />
            )}
          </p>
          <p className={css.modalHelperText}>
            <FormattedMessage id="AuthenticationPage.fixPhoneNumber" values={{ fixEmailLink }} />
          </p>
        </div>
      </div>
    );
    ...
    const verificationContent = isPhoneNumber ? phoneVerificationContent : emailVerificationContent;
        ...
        <LayoutSingleColumn>
          ...
          <LayoutWrapperMain className={css.layoutWrapperMain}>
            <div className={css.root}>
              {showEmailVerification ? verificationContent : formContent}
            </div>
            <Modal
...
const mapDispatchToProps = dispatch => ({
  submitLogin: params => dispatch(login(params)),
