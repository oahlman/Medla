...
const requestSavePhoneNumber = params => (dispatch, getState, sdk) => {
  ...
  const verificationAttempt = params.verificationAttempt;
  const phoneNumberVerified = params.phoneNumberVerified;
...
const verifyPhoneNumber = params => (dispatch, getState, sdk) => {
  return sdk.currentUser
    .updateProfile({
      privateData: {
...
export const saveContactDetails = params => (dispatch, getState, sdk) => {
  ...
  const { email, currentEmail, phoneNumber, currentPhoneNumber, currentPassword, verificationCode, verificationAttempt, phoneNumberVerified } = params;
  const emailChanged = email !== currentEmail;
  const phoneNumberChanged = phoneNumber !== currentPhoneNumber;
  const verificationSuccess = verificationAttempt === verificationCode;

  if (emailChanged && phoneNumberChanged) {
    return dispatch(saveEmailAndPhoneNumber({ email, currentPassword, phoneNumber, verificationAttempt, phoneNumberVerified: false }));
  } else if (emailChanged) {
    return dispatch(saveEmail({ email, currentPassword }));
  } else if (phoneNumberChanged) {
    return dispatch(savePhoneNumber({ phoneNumber, phoneNumberVerified: false }));
  } else if (verificationSuccess) {
    return dispatch(verifyPhoneNumber({ verificationAttempt, phoneNumberVerified: true }));
  } else {
    return dispatch(savePhoneNumber({ phoneNumber, verificationAttempt }));
  }
};
