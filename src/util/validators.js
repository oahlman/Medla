...
export const VERIFYCODE_MIN_LENGTH = 6;
...
const SE_MOBILE_RE = /^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$/;

export const swedenMobileFormatValid = message => value => {
  return value && SE_MOBILE_RE.test(value) ? VALID : message;
};

export const usernameFormatValid = message => value => {
  return value && (SE_MOBILE_RE.test(value) || EMAIL_RE.test(value)) ? VALID : message;
};
